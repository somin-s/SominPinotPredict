...
Arr_Quality_yield = pd.DataFrame(columns=['Quality','Yield','Value','Info','Cluster number','Cluster weight (g)','Shoot number','Vine canopy (%)','Leaf area / metre','Berry weight (g)'])
Arr_Quality_yield_list = pd.DataFrame()
#Arr_model2 = pd.DataFrame()
Arr_model2 = pd.DataFrame(columns=['Berry OD280(AU)','Berry OD320(AU)','Berry OD520(AU)','Juice total soluble solids(oBrix)','Juice pH','Juice primary amino acids(g/L)','Juice malic acid(g/L)',
'Juice tartaric acid(g/L)','Juice calcium(mg/L)','Juice potassium(mg/L)','Juice alanine(μmol/L)','Juice arginine(μmol/L)','Juice aspartic acid(μmol/L)','Juice serine'])
#Arr_model3 = pd.DataFrame()
Arr_model3 = pd.DataFrame(columns=['Wine alcohol(% v/v)','Wine pH','Wine monomeric anthocyanins(mg/L M3G)','Wine total anthocyanin(mg/L M3G)','Wine total phenolics'])
...
for i in range(20):
    ...
    row1 = [source[0], "Yield per wine", source[1], "Information", cn, cw, sn, vc, la, bw]
    ...
    #st.write(Arr_Quality_yield);

    # Arr_Quality_yield = pd.DataFrame(
    # [[source[0], "Yield per wine", source[1], "Information", cn, cw, sn, vc, la, bw], 
    # [source[0], "Yield per metre", source[2], "Information", cn, cw, sn, vc, la, bw],
    # [source[0], "Yield per square metre",source[3], "Information", cn, cw, sn, vc, la, bw]], 
    # columns=['Quality','Yield','Value','Info','Cluster number','Cluster weight (g)','Shoot number','Vine canopy (%)','Leaf area / metre','Berry weight (g)'])
    ...
    row21 = [source[4][0],source[4][1],source[4][2],source[4][3],source[4][4],source[4][5],source[4][6],
    source[4][7],source[4][8],source[4][9],source[4][10],source[4][11],source[4][12],source[4][13]]
    Arr_model2.loc[len(Arr_model2)] = row21
    # Arr_model2 = pd.DataFrame(
    # [[source[4][0],source[4][1],source[4][2],source[4][3],source[4][4],source[4][5],source[4][6],
    #   source[4][7],source[4][8],source[4][9],source[4][10],source[4][11],source[4][12],source[4][13]]],
    # columns=['Berry OD280(AU)','Berry OD320(AU)','Berry OD520(AU)','Juice total soluble solids(oBrix)','Juice pH','Juice primary amino acids(g/L)','Juice malic acid(g/L)',
    # 'Juice tartaric acid(g/L)','Juice calcium(mg/L)','Juice potassium(mg/L)','Juice alanine(μmol/L)','Juice arginine(μmol/L)','Juice aspartic acid(μmol/L)','Juice serine'])
    ...
    row31 = [source[5][0],source[5][1],source[5][2],source[5][3],source[5][4]]
    Arr_model3.loc[len(Arr_model3)] = row31
    # Arr_model3 = pd.DataFrame(
    # [[source[5][0],source[5][1],source[5][2],source[5][3],source[5][4]]],
    # columns=['Wine alcohol(% v/v)','Wine pH','Wine monomeric anthocyanins(mg/L M3G)','Wine total anthocyanin(mg/L M3G)','Wine total phenolics'])
...
#==============================================================================================================================plot graph
#category = px.scatter(Arr_Quality_yield, x="A", y="C", color="B", trendline= "ols", hover_name='D', hover_data=["E","F","G","H","I"])
...
#plot = px.scatter(Arr_Quality_yield, x="A",y="C", color="B", facet_col="B",hover_name='D', hover_data=["E","F","G","H","I"])
